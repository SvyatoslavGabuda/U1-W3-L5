...
const isTodayMyBirthday = (giorno, mese) => {
  const dataOggi = new Date();
  const giornoDelMese = dataOggi.getDate();
  const meseCorrente = dataOggi.getMonth() + 1;

  if (giorno === giornoDelMese && mese === meseCorrente) {
    return true;
  } else {
    return false;
  }

  // return giorno === giornoDelMese && mese === meseCorrente ? true : false;
};

console.log(isTodayMyBirthday(16, 12));
console.log(isTodayMyBirthday(12, 10));
...
console.log("prima", countMovies(movies), "Film");
